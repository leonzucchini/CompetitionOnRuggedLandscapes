FunctionPayoffs = function (J, current.locations, current.prices, 
  ...
  # current.locations = configs.locations; current.prices = configs.prices;
  ...
  if (is.null(dim(current.locations))){# Calculate for single configuration (for final positions)
    ...
    dist = matrix(distances[current.locations,],F,LN) #distances from current position to all points (lookup table)
    e.utility.matrix = exp(Alpha - tau.val * dist - current.prices) #calculate customers' utility from each firm
    if(F>1){divisors  = colSums(e.utility.matrix)}else{divisors = c(e.utility.matrix)}
    ...
    dist = distances[t(current.locations),] #rows 1:F = firm 1's beliefs on distances
    ...
    e.utility.array = array(e.utility.matrix, c(F, F, LN)) #first column (2nd dim) is firm 1's beliefs
    divisors = apply(e.utility.array, c(2,3), sum) #sum of all e(.) terms for each customer, according to each firm's beliefs
    shares = e.utility.array / rep(1,F) %o% divisors #share of customers for each firm, according to each firm's beliefs
    ...
    counter = (0:(F-1))*F+(1:F) # ...to select own payoffs (only these are relevant for move decision)
