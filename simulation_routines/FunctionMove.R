FunctionMove = function(J, current.locations, current.prices, current.payoffs,
  ...
  # current.locations = locations; current.prices = prices; current.payoffs = payoffs
  ...
  rand.pick = ceiling(runif(FJ,0,dim(permissible.moves)[2])) #select column of permissible.moves)
  test.locations = permissible.moves[cbind(current.locations,rand.pick)] #select test location
  configs.locations = rep(1,FJ) %o% current.locations #duplicate current locations
  ...
  test.prices = current.prices + price.step * runif(FJ,-1,1) #select test price within price.step
  test.prices[test.prices<0] = 0
  configs.prices = rep(1,FJ) %o% current.prices #duplicate
  ...
  test.payoffs = FunctionPayoffs(FJ, test.locations, test.prices, 
